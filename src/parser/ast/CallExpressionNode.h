    ...
    virtual void generateTCOExpressionByteCode(ByteCodeBlock* codeBlock, ByteCodeGenerateContext* context, ByteCodeRegisterIndex dstRegister) override
    {
        ...
        if (isSuperCall) {
            ...
            /*
               ...
               try operation
               isConstructCall
               arrow async generator function
               should clear stack
               interpreter self argument make the overall performance slow?
               */
            if (dstRegister == context->m_returnRegister) {
                // Try tail recursion optimization (TCO)
                if (context->m_codeBlock->isTailRecursionTarget()) {
                    codeBlock->pushCode(TailRecursionCallWithReceiver(ByteCodeLOC(m_loc.index), receiverIndex, calleeIndex, argumentsStartIndex, m_arguments.size()), context, this->m_loc.index);
                } else {
                    codeBlock->pushCode(TailCallWithReceiver(ByteCodeLOC(m_loc.index), receiverIndex, calleeIndex, argumentsStartIndex, m_arguments.size()), context, this->m_loc.index);
                }
            } else {
                codeBlock->pushCode(CallFunctionWithReceiver(ByteCodeLOC(m_loc.index), receiverIndex, calleeIndex, argumentsStartIndex, dstRegister, m_arguments.size()), context, this->m_loc.index);
            }
            ...
            if (dstRegister == context->m_returnRegister) {
                // Try tail recursion optimization (TCO)
                if (context->m_codeBlock->isTailRecursionTarget()) {
                    codeBlock->pushCode(TailRecursionCall(ByteCodeLOC(m_loc.index), calleeIndex, argumentsStartIndex, m_arguments.size()), context, this->m_loc.index);
                } else {
