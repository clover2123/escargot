...
#endif
        resultValue = Interpreter::interpret(codeExecutionState, nullptr, byteCodeBlock, reinterpret_cast<size_t>(byteCodeBlock->m_code.data()), registerFile);
        clearStack<512>();
...
Value Script::executeLocal(ExecutionState& state, Value thisValue, InterpretedCodeBlock* parentCodeBlock, bool isStrictModeOutside, bool isEvalCodeOnFunction)
{
    ...
    Value resultValue = Interpreter::interpret(&newState, nullptr, byteCodeBlock, reinterpret_cast<size_t>(byteCodeBlock->m_code.data()), registerFile);
    clearStack<512>();
...
Script::ModuleExecutionResult Script::moduleExecute(ExecutionState& state, Optional<PromiseReaction::Capability> capability)
{
    ...
    if (LIKELY(!m_topCodeBlock->isAsync())) {
        try {
            Interpreter::interpret(newState, nullptr, byteCodeBlock, reinterpret_cast<size_t>(byteCodeBlock->m_code.data()), registerFile);
        } catch (const Value& e) {
            resultValue = e;
            gotException = true;
        }
