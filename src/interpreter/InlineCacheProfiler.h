...
public:
    ...
    // Baseline profiling (inline caching fully disabled in this build).
    // Records one property access at a get/set site. Get and set sites are
    // kept fully separate: every counter, histogram, and summary section is
    // reported per kind.
    //   depth: number of hidden classes (structures) examined until the lookup
    //          concluded — 1 = own property, 2 = first prototype, ...
    //   found: false if the property is absent on the whole chain
    //          (get: undefined read / set: property add), in which case
    //          depth == length of the whole chain that had to be examined
    //   receiverStructure: the receiver's hidden class at this access —
    //          identity only, never dereferenced. Drives the per-site
    //          polymorphism metrics: distinct-structure count (saturating at
    //          33 = megamorphic threshold + 1) and the consecutive-same-
    //          structure ratio (temporal locality; MRU-design evidence)
    //   isLength: the site reads/writes the "length" property (dedicated-
    //          opcode design evidence)
    //   foundIndex: slot index at the holder when found (SIZE_MAX otherwise);
    //          the <= 255 share justifies the Simple tier's uint8 index field
    static void recordAccess(SiteKind kind, void* codePtr, ByteCodeBlock* block, String* propertyName,
...
#define ESCARGOT_IC_PROFILE_RECORD_ACCESS(kind, codePtr, block, propertyName, depth, found, receiverStructure, isLength, foundIndex) \
    ::Escargot::InlineCacheProfiler::recordAccess(::Escargot::InlineCacheProfiler::kind, codePtr, block, propertyName, depth, found, receiverStructure, isLength, foundIndex)
