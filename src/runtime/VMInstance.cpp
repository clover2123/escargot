...
VMInstance::~VMInstance()
{
    ...
    // remove gc event callback
    {
        ...
        Optional<GCEventListenerList::EventListenerVector*> msListeners = list.markStartListeners();
        ASSERT(msListeners.hasValue());
        auto iter = std::find(msListeners->begin(), msListeners->end(), std::make_pair(vmMarkStartCallback, (void*)this));
        ASSERT(iter != msListeners->end());
        msListeners->erase(iter);

        Optional<GCEventListenerList::EventListenerVector*> reListeners = list.reclaimEndListeners();
        ASSERT(reListeners.hasValue());
        iter = std::find(reListeners->begin(), reListeners->end(), std::make_pair(vmReclaimEndCallback, (void*)this));
